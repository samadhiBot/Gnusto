...
struct VerbTests {
    ...
    @Test("Deprecated rawValue initializer works but has empty intents")
    func testDeprecatedRawValueInitializer() throws {
        let verb = Verb(rawValue: "deprecated")
        #expect(verb.rawValue == "deprecated")
        #expect(verb.intents.isEmpty)
    }
    ...
    func testVocalExpressionVerbIntents() throws {
        let vocalVerbs: [Verb] = [
            .tell, .yell, .shout, .scream, .holler, .shriek,
            .laugh, .giggle, .chuckle, .snicker, .chortle,
            .cry, .sob, .weep, .curse, .swear, .damn, .fuck, .shit,
            .sing, .hum, .dance, .xyzzy, .inform,
        ]
    ...
    func testMultiIntentVerbs() throws {
        ...
        #expect(Verb.throw.intents.contains(.throwObject))
    ...
    @Test("All throwing verbs have throwObject intent")
    func testThrowingVerbIntents() throws {
        let throwingVerbs: [Verb] = [.throw, .toss, .chuck, .hurl]

        for verb in throwingVerbs {
            #expect(
                verb.intents.contains(.throwObject),
                "Throwing verb \(verb.rawValue) should have .throwObject intent")
        }
    }
